...
namespace WotN.Minimap
{
    public class MinimapScanner : MonoBehaviour
    {
        ...
        public class CustomInspector : UnityEditor.Editor
        {
            ...
            private Rect GetInspectorWindowSize()
            {
            ...
            public override void OnInspectorGUI()
            {
                ...
                //Support reminder
                GUILayout.Space(10);
                EditorGUILayout.HelpBox("Creating a minimap renderer for your minimap is as simple as that :)", MessageType.None);
                ...
                switch (script.currentTab)
                {
                    case 0:
                        ...
                        Texture2D removeItemIcon = (Texture2D)AssetDatabase.LoadAssetAtPath("Assets/GFX/Textures/Remove.png", typeof(Texture2D));
                        EditorGUILayout.BeginHorizontal();
                        EditorGUILayout.LabelField("GameObjects To Ignore", GUILayout.Width(145));
                        GUILayout.Space(this.GetInspectorWindowSize().x - 145);
                        EditorGUILayout.LabelField("Size", GUILayout.Width(30));
                        EditorGUILayout.IntField(script.gameObjectsToIgnore.Count, GUILayout.Width(50));
                        EditorGUILayout.EndHorizontal();
                        GUILayout.BeginVertical("box");
                        gameObjectsToIgnore_ScrollPos = EditorGUILayout.BeginScrollView(gameObjectsToIgnore_ScrollPos, GUIStyle.none, GUI.skin.verticalScrollbar, GUILayout.Width(GetInspectorWindowSize().x), GUILayout.Height(100));
        ...
        public void DoScanInThisAreaOfComponentAndShowOnMinimap()
        {
            ...
            if (Application.isPlaying == false)
            {
                ...
                //Create base folders
                if (!AssetDatabase.IsValidFolder("Assets/GFX/Textures/Minimaps/"))
                    AssetDatabase.CreateFolder("Assets/GFX/Textures/Minimaps/", "Chapter 1");
                if (!AssetDatabase.IsValidFolder("Assets/GFX/Textures/Minimaps/Chapter 1"))
                    AssetDatabase.CreateFolder("Assets/GFX/Textures/Minimaps/Chapter 1", "Lorencia Area");

                //Generate a name for the scan file
                DateTime date = DateTime.Now;
                pathForFile = "Assets/GFX/Textures/Minimaps/Chapter 1/Lorencia Area/Scan (" + date.Year + date.Month + date.Day + date.Hour + date.Minute + date.Second + date.Millisecond.ToString() + ").png";
            ...
            minimapScanHolder = GameObject.Find("MinimapScanHolder").transform;

            tempSpriteObj = new GameObject("Minimap Scan");
            tempSpriteObj.transform.SetParent(minimapScanHolder);
            tempSpriteObj.transform.position = new Vector3(this.gameObject.transform.position.x, 0, this.gameObject.transform.position.z);
            ...
            tempSpriteObj.transform.position = new Vector3(this.gameObject.transform.position.x + GetSelectedScanArea() / 2, 0, this.gameObject.transform.position.z + GetSelectedScanArea() / 2);
